...
add_task(async () => {
  ...
  //Rooms & Resources are more likely to exist in the history addressbook
  for (let name of ["abe", "kat", "midas", "alain"]) {
    let card = Cc["@mozilla.org/addressbook/cardproperty;1"].createInstance(
      Ci.nsIAbCard
    );
    card.displayName = name;
    card.isRoomResource = true;
  ...
  checkRows(cards.abe, cards.alain, cards.danielle, cards.kat, cards.katherine, cards.midas, cards.natalie, cards.susanah);
